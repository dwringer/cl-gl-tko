...
(let ((rot 0)
  ...
  (defun draw* ()
	...
	(gl:end)
	(gl:flush)
	(glt:window-swap-buffers *gl-canvas-1*)))
    (tk:after 8 #'(lambda ()
		    (glt:window-gl-set-current *gl-canvas-2*)
		    (draw2*))))
  ...
  (defun draw2* ()
	...
	(gl:end)
	(gl:flush)
	(glt:window-swap-buffers *gl-canvas-2*)))
    (tk:after 8 #'(lambda ()
		    (glt:window-gl-set-current *gl-canvas-1*)
		    (draw*)))))
...
(defun test-glt* ()
  (with-main-window (root win) (:width 1280 :height 1024)
	...
	(with-mfd win (mfd center-frame)
	  ...
	  (setf *gl-canvas-1* (canvas :parent center-frame))
	  (pack *gl-canvas-1* :expand t :fill "both" :side "left")
	  (setf *gl-canvas-2* (canvas :parent center-frame))
	  (pack *gl-canvas-2* :expand t :fill "both" :side "right")
	  (glt:window-bind-gl        *gl-canvas-2*)
	  (glt:window-gl-set-current *gl-canvas-2*)
	  (setup-gl-rendering-options)
	  (glt:window-bind-gl        *gl-canvas-1*)
	  (glt:window-gl-set-current *gl-canvas-1*)
	  (setup-gl-rendering-options)
...
(defun install ()
  (save-lisp-and-die "run2.exe"
		     :toplevel #'glt::test-glt*
		     :executable t
		     :application-type :gui))

(defun install-console ()
  (save-lisp-and-die "runConsole2.exe"
		     :toplevel #'glt::test-glt*
		     :executable t))
